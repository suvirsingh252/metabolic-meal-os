...
4. `/settings` can also call `GET /api/diagnostics/notion-schemas` to inspect safe schema summaries, including relation target database/data-source IDs for relation properties and Meal Plan setup warnings.
...
- If `APP_AUTH_TOKEN` is missing, middleware and guarded routes return 503 unless `ALLOW_UNAUTHENTICATED=true` is set. That flag is an explicit trusted family/beta open mode and bypasses app auth even when `APP_AUTH_TOKEN` is present. The legacy `PRIVATE_DEPLOYMENT_MODE=false` opt-out still works for one release and logs a deprecation warning.
...
- `POST /api/intake/share` authenticates against `IOS_SHORTCUT_TOKEN` (bearer or `x-app-auth-token` header; cookies not accepted), is rate limited (20/min/IP), and validates `sharedAt` as ISO-8601. It still requires `IOS_SHORTCUT_TOKEN` when `ALLOW_UNAUTHENTICATED=true`.
